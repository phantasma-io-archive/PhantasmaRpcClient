    ...
    public class RpcApi : IRpcApi
    {
        ...
        public RpcResponse RpcPost (RpcRequest body)
        {
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling RpcPost: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling RpcPost: " + response.ErrorMessage, response.ErrorMessage);
