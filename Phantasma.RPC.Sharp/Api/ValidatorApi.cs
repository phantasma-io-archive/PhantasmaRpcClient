    ...
    public class ValidatorApi : IValidatorApi
    {
        ...
        public List<ValidatorResult> GetValidators()
        {
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException((int)response.StatusCode, "Error calling GetValidatorsGet: " + response.Content,
                    response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException((int)response.StatusCode,
                    "Error calling GetValidatorsGet: " + response.ErrorMessage, response.ErrorMessage);

            return (List<ValidatorResult>)ApiClient.Deserialize(response.Content, typeof(List<ValidatorResult>),
                response.Headers);
        }
    }
}